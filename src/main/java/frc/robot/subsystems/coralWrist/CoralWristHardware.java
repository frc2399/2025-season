...
public class CoralWristHardware implements CoralWristIO {
        ...
        public void setGoalAngle(Setpoint setpoint) {
                ...
                goalState.position = desiredAngle.in(Radians);
        }
        ...
        public boolean atGoal() {
                return (Math.abs(coralIntakeWristRelativeEncoder.getPosition()
                                - goalState.position) < WRIST_ANGLE_TOLERANCE
                                                .in(Radians));
        }

  @Override
  public void calculateNextIntermediateSetpoint() {
        intermediateSetpointState = wristTrapezoidProfile.calculate(0.02, intermediateSetpointState, goalState);
        coralIntakeWristClosedLoopController.setReference(
                intermediateSetpointState.position, ControlType.kPosition, ClosedLoopSlot.kSlot0,
                coralWristFeedFoward.calculate(coralIntakeWristRelativeEncoder.getPosition()
                 + WRIST_ANGULAR_OFFSET.in(Radians), intermediateSetpointState.velocity)
        );
  }

        @Override
        public void resetToCurrentPosition() {
                intermediateSetpointState.position = coralIntakeWristRelativeEncoder.getPosition();
